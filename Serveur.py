...
class Game:
    def __init__(self) -> None:
        ...
        self.listJoueurs = [Joueur(i, self.serveur.team_name[i], self.positionJoueur[i], self.colorJoueur[i], NBCASES)
                            for i in range(len(self.serveur.players))]
...
class Interface(Game):
    ...
    def end_game(self):
        self.render()
        self.display.blit(self.font.render("Terminé !", True, (255, 0, 0)),
                          (WIDTH - self.font.size("Terminé !")[0] - 100, HEIGHT / 2))
        pygame.display.update()
        ...
        for joueur in self.listJoueurs:
            for i in range(NBCASES):
                for unite_id, unite in joueur.list_unite.items():
                    ...
                    size = unite.size // 4 + 1
                    for i in range(size):
                        self.display.blit(pygame.transform.scale(self.sprite_soldier, (SIZE_CASE / size, SIZE_CASE)),
                                          (x + i * SIZE_CASE / size, y))

                    self.display.blit(self.font_unite.render(str(unite.size), True, joueur.color),
                                      (x + (SIZE_CASE / 3), (y + (SIZE_CASE / 2))))

            self.display.blit(self.font.render("{!r}".format(joueur), True, joueur.color),
                              (WIDTH - 450, 25 + joueur.id * 75))
            ...
            for case in row:
                ...
                for lien in case.liens:
                    ...
                    pygame.draw.rect(self.display, RED,
                                     list(map(int, (draw_x, draw_y, SIZE_LIEN * lien.width[0] + off[0],
                                                    SIZE_LIEN * lien.width[1] + off[1]))))

                if not case.function:
                    pygame.draw.rect(self.display, BLUE, list(map(int, (x, y, SIZE_CASE, SIZE_CASE))))
                else:
                    self.display.blit(
                        pygame.transform.scale(self.dicoSpriteCaseFunction[case.function], (SIZE_CASE, SIZE_CASE)),
                        (x, y))
                    if case.function == CASE_FUNCTION.TELEPORT:
                        pygame.draw.rect(self.display, case.pastille,
                                         list(map(int, (x, y, SIZE_CASE / 8, SIZE_CASE / 8))))
